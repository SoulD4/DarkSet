...
import { doc, getDoc, getDocs, collection, orderBy, query, limit } from 'firebase/firestore';
  ...
  useEffect(()=>{
    return onAuthStateChanged(auth, async u=>{
      ...
      try {
        ...
        if(uSnap.exists()){
          const d=uSnap.data();
          const name=(d.name||d.displayName||u.displayName||'Atleta').split(' ')[0];
          setUserName(name); setUserInitials(name.slice(0,2).toUpperCase());
        }
      } catch(_){}
      // Rank global
      try {
