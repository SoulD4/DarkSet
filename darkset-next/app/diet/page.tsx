...
const load = (k: string, fb: unknown = null): unknown => { try { const v = localStorage.getItem(k); return v ? JSON.parse(v) : fb; } catch { return fb; } };
const save = (k: string, v: unknown): void => { try { localStorage.setItem(k, JSON.stringify(v)); } catch {} };
const num  = (v: unknown): number => { const n = parseFloat(String(v).replace(',', '.')); return isFinite(n) ? n : 0; };
const pct  = (v: number, g: number): number => Math.min(100, g > 0 ? Math.round(v / g * 100) : 0);
  ...
  return (
    <div style={{ flex: 1 }}>
      ...
      <div style={{ height:4, background:'rgba(255,255,255,.07)', borderRadius:4, overflow:'hidden' }}>
        <div style={{ height:'100%', width:`${pct(val,goal)}%`, background:color, borderRadius:4, transition:'width .4s' }} />
      </div>
    </div>
  );
}
...
export default function DietPage() {
  const [diet,      setDietState]  = useState<DietLog>(() => (load(LS_DIET, {}) as DietLog));
  const [goals,     setGoalsState] = useState<Goals>(()  => (load(LS_GOALS, { calories:2000, protein:150, carbs:250, fat:65 }) as Goals));
  const [viewDate,  setViewDate]   = useState(todayStr());
  const [tab,       setTab]        = useState<'log'|'semana'>('log');
  const [showAdd,   setShowAdd]    = useState(false);
  const [showGoals, setShowGoals]  = useState(false);
  const [editIdx,   setEditIdx]    = useState<number|null>(null);
  const [form,      setForm]       = useState<Meal>(blankForm());
  ...
  const weekData = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const cal = (diet[k] ?? []).reduce((s: number, m: Meal) => s + num(m.calories), 0);
      days.push({ label: ['D','S','T','Q','Q','S','S'][d.getDay()], cal, isToday: i === 0 });
    }
    return days;
  }, [diet]);
  ...
  const setField = (k: keyof Meal) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
    ...
    <PageShell>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'1.25rem' }}>
        <div>
          ...
          {streak > 1 && <div style={{ fontSize:'.62rem', color:'#f59e0b', marginTop:2, fontWeight:700 }}>🔥 {streak} dias consecutivos</div>}
        </div>
        <button onClick={() => setShowGoals(true)} className="ds-btn ds-btn-ghost" style={{ fontSize:'.72rem', padding:'.35rem .8rem' }}>⚙ Metas</button>
      </div>

      {/* TABS */}
      <div className="mode-toggle" style={{ marginBottom:'1rem' }}>
        {(['log','semana'] as const).map(t => (
      ...
      <div style={{ display:'flex', alignItems:'center', gap:'.5rem', marginBottom:'1rem' }}>
        ...
        <button onClick={() => navigate(1)} disabled={viewDate >= todayStr()} style={{ background:'rgba(255,255,255,.05)', border:'1px solid #222228', borderRadius:8, width:34, height:34, color: viewDate >= todayStr() ? '#2a2a30' : '#f0f0f2', cursor: viewDate >= todayStr() ? 'default' : 'pointer', fontSize:'1rem' }}>›</button>
      </div>
      ...
      {tab === 'log' && (
        <>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'.65rem' }}>
            ...
            <button onClick={openAdd} className="ds-btn ds-btn-primary" style={{ fontSize:'.72rem', padding:'.35rem .85rem' }}>+ Adicionar</button>
          </div>
                  ...
                  <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:'.5rem' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:'.6rem', flex:1, minWidth:0 }}>
                      ...
                      <div style={{ minWidth:0 }}>
                        ...
                        <div style={{ fontSize:'.65rem', color:'#5a5a6a', marginTop:2 }}>{m.time}{m.note ? ` · ${m.note}` : ''}</div>
                      </div>
                    </div>
                    <div style={{ textAlign:'right', flexShrink:0 }}>
                      <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:'1.05rem', color:'#e31b23' }}>
                        {m.calories || '–'} <span style={{ fontSize:'.6rem', color:'#5a5a6a' }}>kcal</span>
                      </div>
          ...
          <div style={{ display:'flex', alignItems:'flex-end', gap:4, height:80 }}>
            {weekData.map((d, i) => (
              <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:3 }}>
                <div style={{ width:'100%', borderRadius:'3px 3px 0 0', background: d.isToday ? '#e31b23' : 'rgba(255,255,255,.15)', height: Math.max(3, Math.round(d.cal / maxCal * 64)), transition:'height .3s' }} />
                <div style={{ fontSize:'.55rem', color: d.isToday ? '#e31b23' : '#5a5a6a' }}>{d.label}</div>
              </div>
            ))}
          </div>
      ...
      {showAdd && (
        <Modal onClose={() => setShowAdd(false)}>
          ...
          <div style={{ display:'flex', gap:'.5rem', marginBottom:'.65rem' }}>
            <select value={form.icon} onChange={setField('icon')}
            ...
            <input placeholder="Nome da refeição" value={form.name} onChange={setField('name')}
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'.5rem', marginBottom:'.65rem' }}>
            {([['calories','Calorias','kcal'],['protein','Proteína','g'],['carbs','Carbs','g'],['fat','Gordura','g']] as [keyof Meal,string,string][]).map(([k,l,u]) => (
              <div key={k}>
                ...
                <input type="number" value={form[k]} placeholder="0" onChange={setField(k)}
          ...
          <div style={{ marginBottom:'1rem' }}>
            ...
            <input placeholder="Ex: grelhado, sem sal..." value={form.note} onChange={setField('note')}
      ...
      {showGoals && (
        <Modal onClose={() => setShowGoals(false)}>
          ...
          {([['calories','Calorias','kcal'],['protein','Proteína','g'],['carbs','Carboidratos','g'],['fat','Gordura','g']] as [keyof Goals,string,string][]).map(([k,l,u]) => (
