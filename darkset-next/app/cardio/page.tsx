...
const calcPace = (d,s) => { if(!d||!s) return '--:--'; const p=(s/60)/d; return Math.floor(p)+':'+String(Math.round((p%1)*60)).padStart(2,'0')+'/km'; };
...
export default function CardioPage() {
  ...
  useEffect(()=>{
    ...
    if(!navigator.geolocation){setGpsInfo('GPS não disponível');return;}
    setGpsInfo('Aguardando GPS...');
    watchRef.current=navigator.geolocation.watchPosition(
      ({coords:{latitude:lat,longitude:lng,accuracy}})=>{
        if(accuracy>50) return;
        setGpsInfo('Rastreando');
        if(lastRef.current){
  ...
  const iniciar=(t)=>{
    ...
    setGpsInfo('');lastRef.current=null;tsRef.current=null;
    setView('sessao');
  };
  ...
  if(view==='sessao'&&tipo) return (
    <PageShell>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'1rem'}}>
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:'1.6rem',textTransform:'uppercase',color:'#f0f0f2'}}>{tipo.icon} {tipo.nome}</div>
      ...
      <div style={{background:'#0e0e11',border:'1px solid #202028',borderRadius:'16px',padding:'1.5rem',textAlign:'center',marginBottom:'.75rem'}}>
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:'5rem',letterSpacing:'.04em',lineHeight:1,color:running?'#e31b23':'#f0f0f2',textShadow:running?'0 0 30px rgba(227,27,35,.35)':'none',transition:'all .3s'}}>{fmt(elapsed)}</div>
        <div style={{fontSize:'.6rem',color:'#5a5a6a',textTransform:'uppercase',letterSpacing:'.1em',marginTop:'.25rem'}}>tempo</div>
        {distancia>0&&(
      ...
      {tipo.gps&&(
        <div style={{background:'#0e0e11',border:'1px solid #202028',borderRadius:'12px',padding:'.85rem 1rem',marginBottom:'.75rem'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            <span style={{fontSize:'.65rem',color:'#5a5a6a',textTransform:'uppercase',letterSpacing:'.07em'}}>📍 GPS</span>
          ...
          {useGPS&&<div style={{marginTop:'.5rem',fontSize:'.78rem',color:gpsInfo==='Rastreando'?'#22c55e':gpsInfo.includes('negada')?'#f87171':'#9898a8'}}>
            {gpsInfo==='Rastreando'?'✅ Rastreando — '+distGPS.toFixed(2)+'km':gpsInfo||'⏳ Aguardando GPS...'}
          </div>}
  ...
  return (
    <PageShell>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'.6rem',marginBottom:'1rem'}}>
        {TIPOS.map(t=>(
          <button key={t.id} onClick={()=>iniciar(t)} style={{background:'#0e0e11',border:'1px solid #202028',borderRadius:'12px',padding:'1rem .5rem',display:'flex',flexDirection:'column',alignItems:'center',gap:'.4rem',cursor:'pointer',transition:'all .12s'}}>
            <span style={{fontSize:'1.8rem'}}>{t.icon}</span>
            <span style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:'.78rem',textTransform:'uppercase',color:'#f0f0f2'}}>{t.nome}</span>
            {t.gps&&<span style={{fontSize:'.52rem',color:'#e31b23',fontWeight:700}}>📍 GPS</span>}
          </button>
        ))}
      </div>

      <div style={{background:'#0e0e11',border:'1px solid rgba(227,27,35,.2)',borderLeft:'2px solid #e31b23',borderRadius:'12px',padding:'1rem'}}>
        <div style={{fontSize:'.65rem',color:'#5a5a6a',textTransform:'uppercase',letterSpacing:'.07em',marginBottom:'.4rem'}}>Último cardio</div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          <div>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:'1.1rem',color:'#f0f0f2'}}>🏃 Corrida</div>
            <div style={{fontSize:'.68rem',color:'#5a5a6a'}}>Hoje · 5.2km · 5:31/km</div>
          </div>
          <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:'1.3rem',color:'#e31b23'}}>28:42</div>
        </div>
      </div>
    </PageShell>
  );
